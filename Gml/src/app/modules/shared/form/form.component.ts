...
import { globalMessage } from 'src/app/modules/shared/message/globalMessage.service';
...
export class FormComponent implements OnInit {
  ...
  public formContact: FormGroup = new FormGroup({
    name: new FormControl('consulta', Validators.required),
    email: new FormControl('consulta@consulta', [Validators.required, Validators.pattern(this.emailPattern)]),
  ...
  private formContactinit=this.formContact.value;
  constructor(private messageOverlayService: globalMessage,private router: Router,
  ...
  public enviar() {
    ...
    this.http.post("https://parseapi.back4app.com/functions/sendEmail", consulta, this.httpOptions).subscribe((data: any) => {
      if (data.result.includes("OK")) {
        this.messageOverlayService.show();
